...
export default function Home() {
  ...
  async function readName(provider){
    try {
      setRetrieveLoader(true)
      ...
      // interact with the methods in smart contract
      const response = await contractWithSigner.readName();
  
      console.log((response))
      setStoredName((response))
      setRetrieveLoader(false)
      return
    } catch (error) {
      alert(error)
  ...
  async function writeName(){
    try {
      ...
      // interact with the methods in smart contract as it's a write operation, we need to invoke the transation usinf .wait()
      const writeNameTX = await contractWithSigner.writeName(enteredName);
      const response = await writeNameTX.wait()
      console.log(await response)
      setStoreLoader(false)

      alert(`Name stored successfully ${enteredName}`)   
      return
  ...
  return (
    <div className='m-6 space-y-4'>
      ...
      <h3>This action retrieves the saved name from smart contract. (i.e Read Operation)</h3>
      <button className='px-4 py-1 bg-slate-300 hover:bg-slate-500 flex justify-around transition-all w-32' onClick={()=>readName(provider)}> { retrieveLoader ? (
                  <svg
                    className="animate-spin m-1 h-5 w-5 text-white"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
      ...
      <h4>The stored name is <span className='font-bold'>{storedName ? storedName : "ABC"}</span> </h4>
      <hr></hr>
...
<h3>This action saves entered Name into the smart contract. (i.e Write Operation) </h3>
      <div>
        <input onChange={(e)=>{
          setEnteredName(e.target.value);
        }} className="placeholder:italic transition-all placeholder:text-gray-500 w-4/6 border border-gray-500 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Enter your name to store" type="text" name="store"/>
      </div>
      <button onClick={writeName} className='px-4 py-1 bg-slate-300 flex justify-around hover:bg-slate-500 transition-all w-32'> { storeLoader ? (
                  <svg
                    className="animate-spin m-1 h-5 w-5 text-white"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
